			...
			<div class="" align="center">
				<table class="container__table" style="text-align: center;">
					...
					<tbody>
						...
						%>
					<tr>
						<td style="width: 50px;"><%=list.get(i).getQuestionId()%></td>
						<td style="width: 700px; text-align: left;"><a style="color: black;"href="service_question_View.jsp?questionId=<%=list.get(i).getQuestionId()%>">
						<%=list.get(i).getQuestionTitle()%></a></td>
						<td style="width: 150px;">익명</td>
						<td style="width: 220px;"><%=list.get(i).getQuestionDate() %></td>
					</tr>
						<%
						}
						%>
				...
				<%if (pageNumber == 1) {%>	
					<input type="button" value="이전" disabled style="background-color:#fbf1e2;" class="mypage_button" id="info_change_now" onclick="location.href='service_question.jsp?pageNumber=<%=pageNumber -1%>'">
			<%	} else {%>
					<input type="button" value="이전" class="mypage_button" id="info_change_now" onclick="location.href='service_question.jsp?pageNumber=<%=pageNumber -1%>'">
			<%  } if (questionDAO.nextPage(pageNumber)) { %>
					<input type="button" value="다음" class="mypage_button" id="info_change_now" onclick="location.href='service_question.jsp?pageNumber=<%=pageNumber +1%>'">
				<%} else { %>
					<input type="button" value="다음" disabled style="background-color:#fbf1e2;" class="mypage_button" id="info_change_now" onclick="location.href='service_question.jsp?pageNumber=<%=pageNumber +1%>'">
				<%} %>
				<br>
				<br>
			<% if (userId != null) {%>
				<div align="center">
					<input type="button" value="글쓰기" class="signUp__btn" onclick="location.href='service_question_write.jsp'">
				</div>
			<%}%>
			</div>
		</div>
